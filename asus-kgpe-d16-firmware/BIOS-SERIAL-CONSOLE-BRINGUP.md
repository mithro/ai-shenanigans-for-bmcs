...
The full operational reference (both control paths, power sequencing, and the
shared-daemon model that avoids multi-reader/writer races) is in
**[`HARDWARE-ACCESS.md`](HARDWARE-ACCESS.md)**. In short: the serial port is owned
by the `seriald` systemd daemon (read `com1.log` with `serialscreen.py`, write via
`serialkey.py` → its FIFO); the USB keyboard (`keysend.py`) and Magewell
(`grabframe.sh`) are `flock`-serialised; power is the Tasmota plug `au-plug-10`.
