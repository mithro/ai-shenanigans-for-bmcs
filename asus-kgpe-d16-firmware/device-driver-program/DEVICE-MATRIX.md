...
**Zephyr @ silicon = 0** and **all silicon = network-BLOCKED from this environment** (the
rig LAN is unroutable here — #150; the drivers are QEMU-validated + the JTAG procedure is
ready for a LAN-connected run); the **5 QEMU ⬜** (DDC/EDID, LPC-mailbox, SOL-mux, PSU-PMBus,
SMBus-ALERT); and Zephyr breadth (11 partial, being extended one validated driver at a
time). This snapshot is regenerated by the tally script; the per-row detail + evidence
citations are below and in [`FULL-TASK-LIST.md`](FULL-TASK-LIST.md).
