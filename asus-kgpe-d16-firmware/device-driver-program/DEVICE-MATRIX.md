...
- **9** LS = 🔷 (blocked): the real vhub EP-DMA datapath on silicon is rig-blocked (patch 0007 is QEMU-verified + compile-clean, but the P2A siphon degrades after ~15 boot cycles; Test B not run to avoid a host-CMOS-halt strand). Earlier `usbip-vudc` gadget-path enumeration is a different, non-vhub route. LU = 🔶 (BMC-side gadget configured; host-side HID enumeration not validated). **§9 scopes this port as "virtual keyboard/mouse/CD" — the HID kbd/mouse gadget is validated (F8-KVM), but the virtual-MEDIA (mass-storage / virtual-CD) function is a distinct, un-validated USB gadget path, now TRACKED as #182 (gate-d 2026-07-20).** D05 / FULL-TASK-LIST B5.
...
- **10** eth0 both sides (NFS-root + Redfish on silicon). UB both = Raptor TFTP. **Model-vs-schematic PHY-part note:** model (`hw/net/ftgmac100.c`) returns RTL8201**CP** PHY-ID, schematic §14 names U5 as RTL8201**N**-GR — a real faithfulness divergence on the MDIO PHY-ID readback (now TRACKED as #181, gate-d 2026-07-20; was "(unresolved)" prose with no task). Low functional risk (both 10/100 RMII) but exactly what a strict oracle diff catches. D06.
...
- **16** W83795 model silicon-seeded (fan1=2641 etc.); hwmon both sides. D08. **ZQ 🔶→✅
  ...
  ZQ-validated. **QE ✅→🔶 (2026-07-20 gate-d/#183 honesty correction):** the model covers register reads +
  the linear PWM→tach DRIVE, but NOT the chip's full feature set — no SmartFan automatic thermal→fan-curve
  mode, no alarm/limit comparison + SMBALERT assertion, no chassis-intrusion (CASEOPEN) latch, no VID. So
  "all functionality" was over-claimed; 🔶 is honest and consistent with row 42 PECI's "not complete
  functionality → 🔶" precedent. Restoring ✅ = model SmartFan/CASEOPEN/VID/alarm (#183). Silicon
  fan-response is a live-hardware follow-on. **23** SB-TSI ZQ 🔶→✅ (2026-07-20):
...
one proven on silicon for engines 1/3/4). **25** SMBus-ALERT (SALT1/2): **scoped 2026-07-19** — the aspeed I2C register header
