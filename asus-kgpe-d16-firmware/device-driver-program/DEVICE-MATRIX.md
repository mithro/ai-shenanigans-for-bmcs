...
justified board-dispositions (no boot-time need). The **real open frontiers** are:
**Zephyr @ silicon = 0** — the rig **IS reachable** (an earlier "LAN-blocked" claim was a
stale SSH-ControlMaster socket, now retracted; see LOG.md), and the honest blocker is the
Zephyr **`__start` data-abort on the JTAG-loaded boot** (#150): DRAM trains + `zephyr.bin`
loads byte-correct on silicon, but the raw JTAG `load_image`+`reg pc`+`resume` doesn't
supply the boot state QEMU `-kernel` implies (exception base / CP15 / CPU mode) — a solvable
entry-state gap, not an environmental wall. Also open: the **5 QEMU ⬜** (DDC/EDID,
LPC-mailbox, SOL-mux, PSU-PMBus, SMBus-ALERT); and Zephyr breadth (11 partial, being extended
one validated driver at a time). This snapshot is regenerated by the tally script; the
