...
**Every device wired in `AST2050-BMC-WIRING.md` is one row below** (42 rows, §§2–15 +
...
grid is 42 × 8 = 336 explicit per-device-per-stack tasks. Machine-counted status (auto-
tally over the rows, 2026-07-19):

| Stack × env | ✅ done | 🔶 partial | 🔷 blocked | ⬜ todo | Ⓝ n/a (justified) |
|---|---|---|---|---|---|
| QEMU emulation | 27 | 7 | 0 | 5 | 3 |
| U-Boot @ QEMU | 10 | 3 | 0 | 3 | 26 |
| U-Boot @ silicon | 8 | 4 | 1 | 3 | 26 |
| Linux @ QEMU | 24 | 7 | 0 | 5 | 6 |
...
justified board-dispositions (no boot-time need). **Zephyr now RUNS ON SILICON** — the
GPIO driver + system timer + VIC boot the real AST2050 over JTAG (banner, `main()`, steady
tickful scheduling; VIC row 36 + Timer row 37 ZS ✅). Getting there fixed four silicon-only
bugs QEMU had hidden (cache/TLB invalidate, VIC ack-at-entry, spurious enable-glitch tick,
entry-addr staleness — commits 918bc7e/b84ef58/78f5569; see LOG.md). The earlier
"LAN-blocked" claim was a stale SSH-ControlMaster socket (retracted). **Remaining frontiers:**
silicon-validate the other Zephyr drivers (I2C/W83795/SB-TSI/WDT) on this now-working base
(#150); the **4 QEMU ⬜** (DDC/EDID, LPC-mailbox, SOL-mux, SMBus-ALERT — PSU-PMBus DONE 2026-07-19); Zephyr
breadth; and an open GPIO-IJKL-readback faithfulness note (silicon reads a floating unwired
pin back as 0 where QEMU models always-1). This snapshot is regenerated by the tally script;
per-row detail + evidence citations are below and in [`FULL-TASK-LIST.md`](FULL-TASK-LIST.md).
