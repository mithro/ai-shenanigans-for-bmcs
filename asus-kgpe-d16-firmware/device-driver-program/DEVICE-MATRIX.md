...
**6 QEMU ⬜** (LPC-BT[4], TPM-passthru[7], DDC/EDID[14], SMBus-ALERT[25], 2D-BitBLT[46], PCI-arbiter[48]); and the broad Zephyr breadth gap
...
| 4 | LPC Ch#3 BT interface (§5 "mailbox") | LPC | ⬜ | Ⓝ | Ⓝ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
...
- **11** **QE ✅→🔶 (2026-07-21 schematic-grounding audit, slice 3 F2):** MAC2 is wired + `net/ncsi` discovers a channel, but only against libslirp's GENERIC single-package NC-SI responder (MFR-ID 0x0, "No GMA handler available"); the FAITHFUL dual-package Intel 82574L responder (OEM MFR-0x157, 2 packages LU1/LU2 per §7, GET_MAC/KEEP_PHY) is UNMODELED — so under "QE = full emulation of ALL functionality" this is 🔶 not ✅, the same over-claim class already downgraded on rows 8/9/16. #204. **UQ/US/LU ⬜→Ⓝ (slice 3 F3):** NC-SI is an OS-level function — U-Boot reaches the network via MAC1 (row 10) and needs no NC-SI driver (UQ/US Ⓝ), and NC-SI has no distinct Linux userspace driver (kernel/netlink-managed → LU Ⓝ). **LS = ⬜ (HARD undone authoring work, NOT externally blocked):** "No channel found" is the G3 RMII2 pinmux group divergence (mainline g4 pinctrl mis-selects RMII2 on the G3; `evidence/d07-ncsi/03-`) — needs the AST2050 RMII2/GPIOE routing RE + a G3 pinctrl group *written* (my code). D07 / FULL-TASK-LIST C2.
...
| # | Device (schematic) | SoC block | QE | UQ | US | LQ | LS | LU | ZQ | ZS |
|---|---|---|---|---|---|---|---|---|---|---|
| 15 | AST2050 I2C controller (7 engines) | I2C | ✅ | ✅ | 🔶 | ✅ | ✅ | ✅ | ✅ | ✅ |
...
- **15** i2cdetect + AC-timing fix proven on silicon. UB-Q = Raptor `libi2c`. D08. **I2C3 (A14/B14) + I2C6 (C12/D12) disposition (2026-07-21, audit slice-2 F2, #207):** these two of the 8 BMC engines get no dedicated device row on purpose — I2C-SMBUS-TOPOLOGY §3.2 shows I2C2/I2C3/I2C6 are three BMC controller taps on ONE shared multi-master sensor bus, whose only slaves are the W83795G hwmon (row 16) + the QU9/QU5 DIMM-mux entry (row 17), both already rowed. The SP5100 on this bus (SMBus1/2) is a **co-master (a bus peer), not a BMC-addressable slave** — verified: the SP5100 presents no BMC-addressable I²C slave here (its own slave-side buses are the separate SMBus0 VR-PMBus + the SR5690-mastered PCIe-hot-plug SMBus, §3.6/§3.8, which the BMC does not drive). So no I²C device is reachable via I2C3/I2C6 that is not already covered by rows 16/17 — no missing row.
...
- **19** DIMM TSOD ×16 (JEDEC TS-on-DIMM, `jc42`, 0x18–0x1F on the QU5 Y2/Y3 banks — schematic §10.2, MUX-FABRIC §5b traces all 16). **DISPOSITION CORRECTED all-Ⓝ→mixed (2026-07-21 schematic-grounding audit, slice 2 F1, #205):** TSOD is a REAL board device (16 sensors reachable via the SAME mux path as the SPDs). The prior "all 8 cells Ⓝ" was over-conservative — it conflated *this bench's TS-less A2 UDIMM* (SPD byte32=0 → 0x19 NAKs on QEMU+silicon) with the DEVICE being n/a, exactly the "falsely-absent" pattern the schematic-grounding pass exists to catch. QEMU controls its own modeled DIMM population, so the QEMU-side cells are REAL WORK. **DONE (2026-07-21, #205): QE/LQ/LU = ✅** — an opt-in machine property `-M kgpe-d16-bmc,ts-dimm=on` (default off = bench-faithful, where 0x19 NAKs) populates a `jc42` TSOD at 0x19 on the QU5 Y2 (DIMM A-D) bank @42000 mC (QEMU submodule f3b9a9bd34), the board DTS declares `temp@19 "jedec,jc-42.4-temp"`, and `scripts/spd-test.py --ts-dimm` drives the full chain (power-on → QU9 → QU5 Y2) so the Linux jc42 binds and userspace reads hwmon `temp1_input=42000` through the mux; the default mode still shows the TSOD absent (no regression). Evidence `evidence/d08-tsod/01`. **UQ/US = Ⓝ** (U-Boot does not read TSOD — OS-level sensor). **LS/ZS = Ⓝ** — the physical bench's TS-less DIMM legitimately gates the silicon cells (a hardware-DIMM property I cannot change remotely; cited). **ZQ = ⬜** (Zephyr jc42 via the mux, remaining). D08.
...
- **40 QE = ✅** — register-accurate `aspeed_pwm_ast2050.c` model @0x1E786000/IRQ28 (fwtest + `integration/test_pwm.py` pass); the only deferred item is tach-RPM synthesis from programmed duty, a low-value refinement since the board never uses the PWM. **The DRIVER stacks (UQ..ZS) are Ⓝ**, not the QE cell: both the VP*/TACH* input balls AND the PWM1/PWM2 output balls (D8/C8) are repurposed as GPIO on this board (`AST_CPU1/2DISABLE#` + the THERMTRIP/PROCHOT/DDR_THERM monitors), and fans run on the W83795G FANCTL, not the AST2050 PWM → board-disposition Ⓝ for every driver stack (2026-07-21 audit slice-1 F1 clarified this note, which previously read ambiguously as if QE were Ⓝ; the QE table cell has always been ✅). D13.
...
| §5 LPC → SP5100/OU1/TPM1 (10) | KCS, BT-mode (§5 "mailbox"), vUART, TPM header | 3, 4, 5, 6, 7 |
...
- **#211 — I²C engine count: 7, not 14 (phantom over-model FIXED 2026-07-21)** (row 15): a gate-a/d
  enumeration audit flagged the row-15 label "8 engines" as contradicting the datasheet. Verified: the
  AST2050/AST1100 datasheet V1.05 says **7** I²C/SMBus controllers, unambiguously and repeatedly
  ("Integrate 7 sets of multi-function I2C/SMBus bus controllers"; "one set of global registers and 7 sets
  of device registers"; SDA7/SCL7 on A12/B12 is the highest). The schematic's "I2C8" is a board-level
  **muxed downstream segment off I2C7** (via QU9/QU5), rowed separately (17 + 26/26b), not a SoC engine.
  But the G3 machine reused `aspeed.i2c-ast2400` (`num_busses=14`), so the model exposed **7 phantom I²C
  engines** the silicon lacks (0x1E78A300+) — the same class of over-model as the removed phantom SRAM/ADC.
  FIXED: new G3-only `aspeed.i2c-ast2050` type (inherits ast2400, caps `num_busses=7`; the 7 engines map at
  0x1E78A040+n*0x40 = 0x1E78A040..0x1E78A1C0, matching silicon), wired on `silicon_rev==AST2050_A1`
  (opt-in-by-connection; AST2400/2500/2600 machines untouched — they legitimately use buses 7/8/11).
  Safe: the kgpe-d16 machine only uses buses 0/1/3/4; boot-validated — all 4 buses register + FRU EEPROM +
  GPIO mux + sensors enumerate identically, no faults (the pre-existing benign `smbus: Unexpected stop`
