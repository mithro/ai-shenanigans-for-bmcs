...
- **14** DDC/EDID: unmodeled (audit gap #7), but now FAITHFULLY SCOPED (2026-07-20, #178). **The
  ...
  *display* controller at all**, and DDC lives in the CRT controller — so there is no register block to
  attach DDC to yet. Faithful path = a CRT-controller register model exposing VGACRB7 DDC-control +
  a downstream EDID (the `hw/display/edid-region.c` / `qemu_edid_region_io` helper already in-tree
  gives a ready EDID blob); oracle-note: the CRT regs sit in the VGA I/O space the C4 vendor firmware
  may touch, so add as a self-contained region + re-boot both oracles. QE stays ⬜ (real work, correctly
  located; NOT Ⓝ — VESA DDC to VGA1 is genuine board function). D12 / #178.
...
- **16** W83795 model silicon-seeded (fan1=2641 etc.); hwmon both sides. D08. **ZQ 🔶→✅
  ...
  baseline 2641, PWM 0x80→3461 rpm, PWM 0x40→1728 rpm (tach TRACKS duty). So row 16 QE "all functionality"
  now covers read AND control; the Zephyr fan-control path is ZQ-validated. Silicon fan-response +
  SmartFan auto-mode are live-hardware follow-ons (not code gaps). **23** SB-TSI ZQ 🔶→✅ (2026-07-20):
...
- **27** power control both sides. **HONESTY CORRECTION (2026-07-18 audit):** power
  ...
  **Zephyr ZS = ⬜ (2026-07-20 gate-d honesty correction, was 🔶):** the `power_smoke`
  ...
  assertion-only).
- **28/29** ~10 §11 signals not yet mapped in DTS `gpio-line-names`. **ZS update (2026-07-20):
