...
| AHB→PCI (P-Bus) Bridge Controller (A2P) | `0x1E72_0000` | 128 KB | 1/2/4 | §9 p97 ("AHB to PCI (P-Bus) Bridge"); §21 p256 | **Yes** | AHB→PCI direction. **NOT a register block — a one-way passthrough WINDOW** (datasheet §21.2, read 2026-07-21): ARM reads/writes in this AHB range are forwarded to P-Bus (PCI) space. Layout: `+0x00000..0x0007F` = relocated I/O on P-Bus (128 B); `+0x00080..0x0FFFF` reserved; `+0x10000..0x1FFFF` = MMIO on P-Bus (64 KB). **Auto-enabled by SCU70[4] (PCI master mode)** — disabled otherwise. The reverse PCI→AHB ("P2A") backdoor is via the PCI **slave** controller BARs (§33). Relevant to the culvert `p2a` path. **QEMU modeling (row 50):** in the standalone BMC machine there is NO host/PCI on the P-Bus, so the faithful model is an explicit A2P window region that is unimplemented/RAZ-WI (forwarding to an empty P-Bus), gated on SCU70[4], replacing the current accidental fall-through — NOT a config-register block. **DDC/EDID (row 14) does NOT depend on this** (that's the CRTC/VGACRB7 bit-bang in the *video* register space, a separate aperture — the earlier "#178 blocks on #176" note conflated the two). |
