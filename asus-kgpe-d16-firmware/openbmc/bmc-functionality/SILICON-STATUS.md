...
| 2 | Connect USB devices | ✅\* | ◐ (port started) | QEMU: `aspeed-vhub` 7-port + mass-storage/HID gadget enumerate over `dummy_hcd` (F6 test PASS). Silicon: enabling the vhub hangs the mainline driver (`usb-vhub-silicon-boundary.txt`); root-caused (CTRL[31] read-only PHY-clock + fatal level ISR[18] deadlock — `VHUB-G3-PORT-PLAN.md`) and **fixed in patch 0007** (PHY-ready gate + de-livelock + `ast2050-usb-vhub` compatible). Patch is compile-clean and **QEMU-verified probe-safe** (F6 PASS). Silicon retest **rig-blocked** (P2A load degrades after ~15 boot cycles). Still needs: a faithful QEMU udc model to reproduce+gate the hang, then a fresh-boot silicon retest; host enumeration also needs a USB-host emulation (QEMU) / physical wiring (silicon). |
