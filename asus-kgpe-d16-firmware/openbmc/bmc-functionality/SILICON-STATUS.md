...
| 2 | Connect USB devices | ✅\* | ✗ (boundary found) | QEMU: `aspeed-vhub` 7-port + mass-storage/HID gadget enumerate over `dummy_hcd`. Silicon: characterized precisely (`evidence/real-hw-consolidated/usb-vhub-silicon-boundary.txt`) — the `aspeed_vhub` driver is in the kernel, the `usb-vhub@1e6a0000` DT node was present-but-disabled with G3 counts; **enabling it hangs the boot** (mainline `ast2400-usb-vhub` probe faults on the G3's different register layout). Real host-facing USB needs a **G3-adapted vhub driver port** — the largest remaining item, not a DT-only fix. |
