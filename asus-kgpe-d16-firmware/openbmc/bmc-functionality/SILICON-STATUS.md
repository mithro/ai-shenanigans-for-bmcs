...
| 9 | Piggyback host NIC | ◐ | ◐ | **REOPENED (D07), 2026-07-18** — the earlier "true NC-SI host-NIC sharing is architecturally ABSENT" verdict was **MAC1-scoped and is superseded**: the schematic netlist (`../../schematic-wiring/AST2050-BMC-WIRING.md` §7) confirms **MAC channel 2's RMII2 (balls A5/B5/B6/C4/D4/D5) IS wired as a multi-drop NC-SI sideband to BOTH Intel 82574L NICs (LU1/LU2)**, 50 MHz REF from clockgen CU2, NICs on +3V3_AUX (fabric doc §5 — so testable with the host off). **Implementation not started** (QEMU MAC2 + NC-SI responder model, Linux `net/ncsi`, silicon validation — task D07 in `../../device-driver-program/TASKLIST.md`; see the F7-NCSI.md 2026-07-18 correction). Still true: MAC1 uses a dedicated RTL8201-family PHY (SCU40[15:14] is a software hint, not the strap), and the BMC is reachable on the shared physical network (both sides). |
