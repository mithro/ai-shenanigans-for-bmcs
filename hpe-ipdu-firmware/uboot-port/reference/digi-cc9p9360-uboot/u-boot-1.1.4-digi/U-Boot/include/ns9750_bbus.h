 ...
 * All rights reserved.
 *
 * $Id: ns9750_bbus.h,v 1.3 2005/01/12 13:05:26 jjaeger Exp $
...
/* We have support for 50 GPIO pins for the NS9750 and
    72 GPIO for the NS9360 */
...
#define get_gpio_stat_reg_addr(pin) \
/*	get_bbus_reg_addr( NS9750_BBUS_GPIO_STAT_BASE + (((pin)<32)?0:4) )*/ \
	get_bbus_reg_addr( NS9750_BBUS_GPIO_STAT_BASE + (((pin) >> 5) * 4) \
					+ (((pin) <64)?0:232))


#define get_gpio_ctrl_reg_addr(pin) \
/*	get_bbus_reg_addr( NS9750_BBUS_GPIO_CTRL_BASE + (((pin)<32)?0:4) )*/ \
	get_bbus_reg_addr( NS9750_BBUS_GPIO_CTRL_BASE + (((pin) >> 5) * 4) \
					+ (((pin) <64)?0:232))

#define set_gpio_ctrl_reg_val(pin,val) \
	(*get_gpio_ctrl_reg_addr(pin)=((*get_gpio_ctrl_reg_addr((pin)) &~(1 << ((pin) % 32))) |\
					((val) << ((pin) % 32))))

/* returns 1 if status bit is 1 otherwise 0 */
#define get_gpio_stat(pin) \
	(((*get_gpio_stat_reg_addr(pin)) & (1 << (((pin) % 32))))>0?1:0)
