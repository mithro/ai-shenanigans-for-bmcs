 ...
 * It has been tested on an Intel Strataflash
 *
 * References
...
static ulong flash_get_size (ulong base, int banknum, ulong *addr);
...
unsigned long flash_init (void)
{
	...
	for (i = 0; i < CFG_MAX_FLASH_BANKS; ++i) {
		...
		size += flash_info[i].size = flash_get_size (bank_base[i], i,(ulong *) PHYS_FLASH_1);
...
void flash_print_info (flash_info_t * info)
{
	...
	printf ("  Erase timeout %ld ms\n  Write timeout %ld ms\n  Buffer write timeout %ld ms\n  Buffer size %d\n",
		info->erase_blk_tout,
		info->write_tout,
...
static int flash_detect_cfi (flash_info_t * info)
{
	...
	for (info->portwidth = FLASH_CFI_8BIT;
		...
		for (info->chipwidth = FLASH_CFI_BY8;
			...
			if (flash_isequal (info, 0, FLASH_OFFSET_CFI_RESP, 'Q') && flash_isequal (info, 0, FLASH_OFFSET_CFI_RESP + 1, 'R') && flash_isequal (info, 0, FLASH_OFFSET_CFI_RESP + 2, 'Y')) {
...
static ulong flash_get_size (ulong base, int banknum, ulong *addr)
{
	...
	// After making CFI Initialisation make normal chip recognition

	/* Write auto select command: read Manufacturer ID */
	addr[0x5555] = (ulong) 0x00AA00AA;
	addr[0x2AAA] = (ulong) 0x00550055;
	addr[0x5555] = (ulong) 0x00900090;
	...
	case (ulong) INTEL_MANUFACT:
		info->flash_id = FLASH_MAN_INTEL;
		break;

	default:
		info->flash_id = FLASH_UNKNOWN;
		addr[0] = (ulong) 0x00FF00FF;	/* restore read mode */
