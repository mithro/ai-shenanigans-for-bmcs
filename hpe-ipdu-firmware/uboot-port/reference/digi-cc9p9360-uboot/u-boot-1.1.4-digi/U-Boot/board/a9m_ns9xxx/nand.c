...
static int cc9p_wait(struct mtd_info *mtd, struct nand_chip *this, int state)
{
	NAND_WAIT_READY();
	return 0;
}

static u_char cc9p_read_byte(struct mtd_info *mtd)
{
...
static void cc9p_read_buf(struct mtd_info *mtd, u_char* buf, int len)
{
	struct nand_chip *this = mtd->priv;
...
static void cc9p_write_byte(struct mtd_info *mtd, u_char byte)
{
	struct nand_chip *this = mtd->priv;
...
static void cc9p_write_buf(struct mtd_info *mtd, const u_char *buf, int len)
{
	struct nand_chip *this = mtd->priv;
...
static void cc9p_cmdfunc(struct mtd_info *mtd, unsigned command,
 ...
 */
static void cc9p_hwcontrol(struct mtd_info *mtd, int cmd)
{
	switch(cmd) {
...
void board_nand_init(struct nand_chip *nand)
{
	...
	nand->hwcontrol = cc9p_hwcontrol;
	nand->eccmode = NAND_ECC_SOFT;
	nand->waitfunc = cc9p_wait;
	nand->read_byte = cc9p_read_byte;
	nand->write_byte = cc9p_write_byte;
	nand->read_buf = cc9p_read_buf;
	nand->write_buf = cc9p_write_buf;

	nand->cmdfunc = cc9p_cmdfunc;
	nand->chip_delay = 12;
