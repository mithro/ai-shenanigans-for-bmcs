...
#define	puts(x)		/* AT91F_UNC90_Printk(x) */
#define	error(x)	recover(x)
...
int copy_image(void *src, void *dst, unsigned int len)
{
	unsigned short int *p_orig = (ush *) src;
	unsigned short int *p_dest = (ush *) dst;
	int num_words = (len+1) >> 1;	// len is in bytes 
