...
int do_bdinfo ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    defined (CONFIG_CCXP) || defined (CONFIG_CC9C)
#if defined (CONFIG_A9M2410) || defined (CONFIG_A9M9750) || \
    defined (CONFIG_A9M9750) || defined (CONFIG_A9M9360)
	for (i=0; i<CONFIG_NR_NAND_PART; ++i) {
		print_num("NAND part.",	i);
		printf ("-> name     = %s\n", bd->bi_nand[i].name);
		print_num("-> start",	bd->bi_nand[i].start);
		print_num("-> size",	bd->bi_nand[i].size);
	}
#endif
	puts  ( "\n"
		"gw_addr     = ");
	print_IPaddr (bd->bi_gw_addr);
	puts  ( "\n"
		"ip_mask     = ");
	print_IPaddr (bd->bi_ip_mask);
	puts  ( "\n"
		"dhcp_addr   = ");
	print_IPaddr (bd->bi_dhcp_addr);
	puts  ( "\n"
		"dns_addr    = ");
	print_IPaddr (bd->bi_dns_addr);
	puts  ( "\n");
#endif
	puts ("ethaddr     =");
	for (i=0; i<6; ++i) {
